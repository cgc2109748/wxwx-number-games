...
class Level3Game {
  ...
  calculateLayout() {
    ...
    const availableHeight = this.canvas.height - this.layout.gridStartY - 120; // 预留按钮空间
  ...
  bindEvents() {
    ...
    if (typeof wx !== "undefined") {
      ...
      this.canvas.addEventListener("click", this.touchHandler);
    }
  }
  ...
  checkElimination() {
    ...
    if (cell1.value + cell2.value === 10) {
      // 检查路径是否畅通
      if (this.isPathClear(cell1.row, cell1.col, cell2.row, cell2.col)) {
        // 消除数字
  ...
  isPathClear(row1, col1, row2, col2) {
    ...
    // 检查直线路径
    return this.checkStraightPath(row1, col1, row2, col2);
  }

  checkStraightPath(row1, col1, row2, col2) {
    // 水平路径
    if (row1 === row2) {
      const startCol = Math.min(col1, col2);
      const endCol = Math.max(col1, col2);
      for (let col = startCol + 1; col < endCol; col++) {
        if (this.grid[row1][col].visible) {
          return false;
        }
      }
      return true;
...
if (typeof global !== "undefined") {
  global.Level3Game = Level3Game;
} else if (typeof window !== "undefined") {
  window.Level3Game = Level3Game;
} else {
  this.Level3Game = Level3Game;
}
